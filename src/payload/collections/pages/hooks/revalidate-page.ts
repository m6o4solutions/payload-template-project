...
/**
 * revalidates the page path and sitemap tag after a page document is deleted.
 */
const revalidateDelete: CollectionAfterDeleteHook<Page> = ({ doc, req: { context } }) => {
	if (!context.disableRevalidate) {
		const path = doc?.slug === "home" ? "/" : `/${doc?.slug}`;

		// assuming you have access to payload.logger here, which might require passing 'payload' via req
